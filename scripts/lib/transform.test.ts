...
import { type SourceData, sourceDataSchema } from "./source-schema.ts";
...
describe("transform on the real vendored data", () => {
	const source = sourceDataSchema.parse(
		JSON.parse(readFileSync("data/vendor/greeny-data1.0.json", "utf8")),
	);

	it("produces the expected entity counts", () => {
		const { data } = transform(source);
		expect(data.items).toHaveLength(175);
		expect(data.recipes).toHaveLength(797);
		expect(data.buildings).toHaveLength(20);
		expect(data.buildables).toHaveLength(480);
		expect(data.schematics).toHaveLength(437);
		expect(data.resources).toHaveLength(13);
		expect(data.generators).toHaveLength(4);
		expect(data.miners).toHaveLength(5);
	});

	// Guards against silent input-coverage regressions: today 99 refs are dropped
	// (86 unlock recipes + 8 required schematics + 5 cost items). A loose upper
	// bound catches a future source update that suddenly drops far more, without
	// being brittle to small data changes.
	it("drops only the known set of dangling references", () => {
